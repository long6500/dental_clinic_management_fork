...
const createAdmin = async (req, res) => {
    ...
    try {
        ...
        const profileAdmin = { _id: "admin", fullname: 'Nguyễn Thành Nam', phone: '0974485920', email: process.env.EMAIL_ADMIN, address: '', userId: admin[0]._id };
...
const getProfile = async (req, res, next) => {
    ...
    const [profile, totalProfile] = await Promise.all([
    ...
    res.send({ success: 1, data: { data: profile, total: totalProfile } });
};

const createProfile = async (req, res) => {
    const senderUser = req.user;
    const { fullname, phone, email, address, workingDays, salary, status, role, schedule } = req.body;
    ...
    if (role != null) {
        ...
        await Promise.all(roleArray.map(async (element) => {
    ...
    if (schedule != null) {
        ...
        await Promise.all(scheduleArray.forEach(async (element) => {
...
const updateProfile = async (req, res) => {
    ...
    const { fullname, phone, email, address, workingDays, salary, status, schedule } = req.body;
...
const getProfileById = async (req, res) => {
    ...
    const schedule = scheduleId.map(async id => await ScheduleModel.findById(id));
...
const updateStatus = async (req, res) => {
    ...
    await UserModel.findByIdAndUpdate(existStaff.userId, {
        status,
        modifyBy: senderUser._id
    }, { new: true });

    const updatedStaff = await ProfileModel
        .findByIdAndUpdate(staffId, {
            status,
            modifyBy: senderUser._id
        }, { new: true });
